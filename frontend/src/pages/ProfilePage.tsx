...
export default function ProfilePage() {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (uploadError) {
      // Storage bucket may not exist — fall back to base64 preview stored in metadata
  ...
  const handleSaveName = async () => {
    ...
    if (!error) {
      ...
      // Force session refresh so all components pick up the new name
  ...
  const handleDeleteAccount = async () => {
    ...
    // Call backend endpoint to delete user with service role key
    try {
      const { data: { session } } = await supabase.auth.getSession()
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/auth/delete-account`, {
        method: 'DELETE',
        headers: {
      ...
      <div style={{ maxWidth: 680, animation: 'fadeIn 0.5s ease forwards' }}>
            ...
            <div>
              ...
              {avatarUrl && (
                <button
                  ...
                  style={{ fontSize: 12, color: t.textMuted, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}
                >
                  Remove photo
                </button>
              )}
              <p style={{ fontSize: 12, color: t.textMuted, marginTop: 6 }}>JPG, PNG, GIF · Max 2MB</p>
            </div>
          </div>
        </Section>
        ...
        <Section title="Danger zone" t={t}>
          <div style={{ border: '1px solid #fecaca', borderRadius: 10, padding: '20px 24px', background: isDark ? 'rgba(239,68,68,0.05)' : '#fff5f5' }}>
            ...
            {deleteError && (
              <p style={{ fontSize: 12, color: '#dc2626', marginTop: 10 }}>⚠ {deleteError}</p>
            )}
          </div>
        </Section>
